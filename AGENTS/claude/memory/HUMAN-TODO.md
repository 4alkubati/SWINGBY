...
- [ ] **(Bucket C — push, 1 min)** Two local commits are ready on main: the KIRA-voice brief + memory sync, and the **two-way date handshake** (client proposes times from their chat → business approves; either side can counter). Gates green: pytest 44/3, babel 115/0, flow graph 0 broken, DB migration already applied. `git push` → Render autodeploys → `python3 tools/e2e_smoke.py https://swingbyy-api.onrender.com` (now includes the handshake steps).
