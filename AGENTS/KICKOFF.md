...
4. **He directs, agents build overnight, he reviews ~2 hrs.** Daily files are written so his review window is decisions, not debugging: WIN CONDITION (1–3 checkboxes) → MORNING/NIGHT pure-action blocks → OVERNIGHT dispatch line → SCORECARD (the one task that counts) → GLOSSARY at the bottom (steps never explain inline).
