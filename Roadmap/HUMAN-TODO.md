...
| H21 | **TODO — NOW BREAKING PRODUCTION** | **Apply the D5 migrations** `20260803120000_stripe_connect_payouts.sql` **and** `20260806040000_payouts_rls.sql` in the Supabase SQL editor. ⚠️ **The old text on this row said "only needed when PR #97 merges" and that was wrong** — the D5 code reached `main` via **PR #98**, not #97, so it is already deployed to Render while its schema does not exist. **Proven live 2026-08-06:** `GET /businesses/me/payouts` returns **500** as an authenticated business (the Wallet screen), and `information_schema` shows **0 of 7** connect columns on `businesses` and no `payouts` table. `GET /businesses/me/payout-account` returns a clean 200 `state: "none"`, which makes the breakage look narrower than it is. Additive and `if not exists` throughout. |
