...
| H31 | **STEP 1 ANSWERED 2026-08-12 — now a Google Cloud task, then one flip** | **Kira: "we do have the google reviews"** — so the long pole (Step 0, Google approving Business Profile API access, *"expect weeks, not days"*) is **done**. ⚠️ Read that as *API access approved*, not *SwingBy has reviews on its listing* — those are different things and only the first unblocks this. If it was the second, this row goes back to TODO. **Verified from here the same day, so nobody re-checks it:** the migration `20260725210000_google_business_reviews_import.sql` **is applied** (all 3 tables live), and RLS matches the doc exactly — `business_google_connections` and `business_google_oauth_states` have **zero** policies (tokens are `service_role`-only, unreadable by any user JWT), `business_imported_reviews` has one `SELECT` for `authenticated` and **no write policy**, so a user JWT cannot manufacture a verified review via PostgREST. All 3 tables are empty. **What is genuinely left, and none of it is in this repo:** (1) Google Cloud → OAuth consent screen with scope `business.manage` (a **sensitive** scope — the app must be published/verified), (2) create the OAuth client and get its id, (3) Render: `GOOGLE_BUSINESS_CLIENT_ID`, `GOOGLE_BUSINESS_REDIRECT_URI`, then `GOOGLE_REVIEWS_ENABLED=1`, restart. **No client id exists anywhere on this box** — I searched; unlike the Telegram bot, this one has not already been created. Omit `GOOGLE_BUSINESS_CLIENT_SECRET` for an installed-app client (it has none, and an empty one is rejected as `invalid_client`). Original note follows. |
| H31-orig | *(superseded, kept for the detail)* | **Google review import.** Probed live 2026-08-12 as `testbusiness@swingby.dev`: `GET /google-reviews/status` → `enabled: false, status: "coming_soon"`, message *"waiting on Google to approve SwingBy's access"*. **The code is complete and has been since 2026-07-25** — turning it on is env vars in Render and a restart, no deploy of new logic. **Step 1 is not the flag, it is confirming Google actually granted Business Profile API access** (Google Cloud console / approval email); the flag has been off because that approval had not landed, and nothing in this repo can tell whether it since has. Then in Render: `GOOGLE_REVIEWS_ENABLED=1`, `GOOGLE_BUSINESS_CLIENT_ID`, `GOOGLE_BUSINESS_CLIENT_SECRET` (omit for an installed-app client), `GOOGLE_BUSINESS_REDIRECT_URI`, `GOOGLE_BUSINESS_REDIRECT_PREFIXES`. **A half-flip fails loudly, not silently** — flag on with no client id returns 503 and logs the reason (`google_reviews.py:227`), so setting the flag alone is safe to get wrong. Full operator guide: `docs/GOOGLE_REVIEWS_SETUP.md`. |
