...
| 2026-07-21 | Docs-vs-reality review pass. Regenerated `docs/API.md` from the live FastAPI OpenAPI schema (40 documented → 80 real operations; fixed booking-events/photos mounts, Stripe checkout path, push-token register, dropped the nonexistent `GET /admin/businesses`, documented the missing `PATCH /bookings/{id}/propose-dates`). Regenerated `docs/swingby_database_schema.md` from live Supabase introspection — 16 tables, real CHECKs/FKs incl. `booking_events.actor_id ON DELETE RESTRICT`; removed the phantom `payments.notes` and `bookings.completed_at`; flagged the dead `service_posts.preffered_date` typo column. Fixed `tools/flow_graph.py`: broken-edge detection is now per-navigator (was a union of all navigators, which hid every cross-navigator break and produced a false "every navigation target resolves") — regenerated graph surfaces 10 real breaks. New `docs/MIGRATIONS.md` documenting the raw-SQL migration process, prod-introspection verification, the same-PR rule, and a payload-key diff checklist (the step that was skipped in the 07-20 sweep and caused the 07-21 invoice outage). Cleared stale "FILE ONLY / pending apply" headers on referrals/preferred_date/geocoding SQL, marked APPLY-2026-07-20.sql applied, and fixed stale comments in geocoding.py, admin.py, NearbyMapScreen.web.js. |
