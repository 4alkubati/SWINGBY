...
## 2026-07-26 — reconciled against the repo and prod

> This file was written 2026-07-21 and then sat on an unmerged branch while
> three days of work shipped, so everything above it predates the walkthrough
> lanes. Rather than trust it, this section was rebuilt by listing
> `supabase/migrations/` and verifying each object against the live database.
> **The standing rule still applies: query prod before trusting any schema doc,
> including this one.**

Migrations now living in `supabase/migrations/` (the authoritative folder —
the older `docs/*.sql` scripts are historical and are not re-applied):

| File | Applied to prod | Verified how |
|---|---|---|
| `20260724090000_proof_of_work_and_auto_bidding.sql` | yes | `booking_proofs`, `booking_voice_notes`, `business_auto_bid_rules` all return 200 via PostgREST |
| `20260725190000_voice_notes_bucket.sql` | yes | `storage.buckets` row `voice-notes`, private, 8 MB cap |
| `20260725210000_google_business_reviews_import.sql` | yes | `business_google_connections`, `business_google_oauth_states`, `business_imported_reviews` |
| `20260725220000_owner_is_the_default_assignee.sql` | yes | `employees` selectable with the new shape |
| `20260725230000_thread_attachments_and_terms.sql` | yes | `message_terms` + `messages.message_type` / `attachment_*` |

All five were applied 2026-07-25/26 and confirmed before `integrate/lanes-2026-07-25`
was merged to `main`, because Render deploys `main` on push — code reaching a
database that lacks its tables is the Jul 17 outage.

`supabase/APPLY-2026-07-25-all-three.sql` is the one-file paste script for the
last three; it is deliberately pure ASCII and every statement is
if-not-exists / on-conflict guarded, so re-running it is harmless.
