    ...
    if (
      ...
      try {
        ...
        if (newToken) {
          ...
          // Return the BODY, not a { data } wrapper. The retry interceptor
          // above re-wraps because THIS interceptor still runs after it and
          // unwraps again — but this is the last interceptor in the chain, so
          // whatever it returns is what the caller receives. Wrapping here
          // handed every post-refresh caller { data: <body> } instead of the
          // body: getMe() would have set user = { data: {...} } and rendered a
          // blank profile. Covered by src/__tests__/auth-refresh.test.js.
          return api(config);
        }
      } catch (refreshErr) {
        _refreshInFlight = null;
