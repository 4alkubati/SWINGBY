# SwingBy Knowledge Base — 30 FAQs

---

## Account & Signup

**Q1: How do I create an account?**  
Go to swingbyapp.com, tap "Sign up," enter your name, email, and password. Choose your role: Client (looking for services) or Business (offering services). You'll receive a confirmation email — click the link to activate your account.

**Q2: Can I be both a client and a business on the same account?**  
Not on the same account. If you want to use SwingBy as both a client and a business owner, create two separate accounts with different email addresses.

**Q3: I didn't receive my confirmation email. What do I do?**  
Check your spam folder first. If it's not there, wait 5 minutes and try again. Still nothing? Email support@swingbyapp.com and we'll manually confirm your account.

**Q4: How do I reset my password?**  
On the login screen, tap "Forgot password." Enter your email and we'll send a reset link. The link expires in 1 hour.

**Q5: Can I change my email address or phone number?**  
Yes — go to your profile settings and update your contact details. A verification email will be sent to confirm the new address before it takes effect.

---

## Posting a Job (Clients)

**Q6: How do I post a job?**  
Tap "Post a Job" on your dashboard. Enter a title, describe what you need, choose a category, set your budget range, and confirm your location. Your post will be visible to matching local businesses immediately.

**Q7: How long does my job post stay active?**  
Job posts expire after 7 days if no booking is confirmed. You can cancel it earlier from your dashboard.

**Q8: Can I post a job without specifying a price?**  
You need to enter a budget range, but it's flexible — you can set a wide range like "$50–$300" if you're not sure what to expect. Businesses will quote within or around your range.

**Q9: Can I edit my job post after publishing?**  
You can add clarifying details in the messages once you've accepted a quote, but you can't edit the original post. If the post needs major changes, cancel it and re-post.

**Q10: How do I choose which business to work with?**  
When businesses express interest in your job, you'll see their profile (photos, reviews, rating) and their quoted price. Take your time — you're under no pressure to accept the first quote.

---

## Getting Quotes & Accepting (Clients)

**Q11: A business sent me a quote. What happens next?**  
Review their profile and quote. If you're happy, tap "Accept." This creates a booking, locks in the price, and holds your payment in escrow. The business is then notified and you can message them to arrange details.

**Q12: What if I accept a quote and then want to change my mind?**  
You can cancel a booking from your dashboard. Cancellation penalties apply depending on how close to the scheduled date you cancel (see: cancellation policy in Q28).

**Q13: Can I negotiate the price with a business before accepting?**  
SwingBy doesn't have a built-in negotiation feature — but you can message a business after they express interest by first accepting their quote. If price is your concern, filter by quoted price before deciding.

**Q14: What if no businesses respond to my job post?**  
Try widening your service radius or adjusting your budget. Some categories have fewer businesses in certain areas. You can also repost with a more detailed description. Contact support if you've had no response after 48 hours.

---

## Payments & Escrow

**Q15: How does payment work?**  
When you accept a quote, your payment is placed in escrow — it's held securely until the job is complete. The business receives 50% upfront as booking confirmation. The remaining 50% is released when you mark the job complete in the app.

**Q16: Is my payment information stored securely?**  
Yes. Payments are processed through Stripe. SwingBy never stores your card details directly — they're held by Stripe under PCI-DSS compliance.

**Q17: When will my card be charged?**  
Your card is charged immediately when you accept a quote and confirm the booking.

**Q18: What payment methods are accepted?**  
Credit and debit cards (Visa, Mastercard, Amex). E-transfer is not supported through the platform.

---

## Messaging

**Q19: Can I message a business before booking?**  
No. Messaging is only available after a booking is confirmed. This protects both parties and keeps transactions within the platform.

**Q20: What if a business asks me to pay outside the app?**  
Do not pay outside the app. Payments made outside SwingBy are not protected by escrow and we cannot help with disputes. Report this request to support@swingbyapp.com.

---

## Business Onboarding

**Q21: How do I set up my business profile?**  
After signing up as a business, go to your dashboard and tap "Set up profile." Add your business name, service category, service area, a description, and photos. Your profile is reviewed before going live.

**Q22: How do I get my license verified?**  
Upload your license/certification documents in your profile settings. The SwingBy team manually verifies these within 2–3 business days. Unverified businesses can still operate but won't show the "Verified" badge.

**Q23: How do I browse and respond to jobs?**  
Your dashboard shows open jobs near you that match your service category. Tap any job to see details, and tap "Express interest" to send a quote.

**Q24: When do I get paid?**  
50% is released to you when a client accepts your quote. The remaining 50% (minus 10% platform cut) is released when the client marks the job complete. Payouts arrive within 2–3 business days via Stripe.

---

## Reviews

**Q25: How do reviews work?**  
After a job is completed, both the client and the business can leave a review. Reviews are visible on public profiles. We do not allow businesses to remove reviews.

**Q26: Can I dispute a review I think is unfair?**  
Contact support@swingbyapp.com with details. We investigate reviews that violate our terms (fake reviews, harassment, clearly fraudulent). We do not remove reviews simply because a business disagrees with the rating.

---

## Disputes & Issues

**Q27: What if the work wasn't done properly?**  
Do not mark the job complete. Contact support@swingbyapp.com within 24 hours of the scheduled completion date. We'll open a dispute and work through our dispute playbook to reach a fair resolution.

**Q28: What is the cancellation policy?**  
- Cancel more than 48 hours before scheduled date: 25% of the job value is retained as a cancellation fee.  
- Cancel within 48 hours: 50% retention fee.  
- No-show (business): client receives full refund, no fee.  
- No-show (client): business retains 25%.

**Q29: What if a business doesn't show up?**  
Contact support immediately. We will issue a full refund from escrow and take action against the business account. Repeated no-shows result in account suspension.

**Q30: How do I close my account?**  
Email support@swingbyapp.com with subject line "Account closure request." We'll confirm any pending bookings and process the closure within 5 business days.
