# Drip: New Client Onboarding (5 emails over 30 days)

> Goal: Get the new client to post their first job and complete their first booking.
> Voice: Warm, direct, helpful. No upsell, no pressure.
> CASL: All emails sent only to users who signed up and opted in. Unsubscribe link in every email.

---

## Email 1 — Day 0 (sent immediately after signup)

**Subject:** Welcome to SwingBy — here's how to get started

**Preview text:** Post your first job in two minutes. Here's how.

---

Hi {{first_name}},

You're on SwingBy — the simplest way to find trusted local service businesses in Calgary.

Here's how it works in three steps:

1. **Post a job.** Describe what you need, set a rough budget, and choose your area. Takes two minutes.
2. **Get quotes.** Vetted local businesses in your area respond with their price and availability.
3. **Book with confidence.** Your payment is held safely in escrow until the job is done to your satisfaction.

**Post your first job free:** [swingby.ca/app/post](https://swingby.ca/app/post)

What services do we cover? House cleaning, handyman work, lawn care, dog walking, personal training, moving help, and more — all in Calgary.

— The SwingBy team

---

## Email 2 — Day 3 (if no job posted)

**Subject:** Still looking for a local service in Calgary?

**Preview text:** It takes two minutes to post a job. Here's what to write.

---

Hi {{first_name}},

Haven't posted a job yet? Here's what a good job post looks like:

> "Need a deep clean for my 2-bedroom condo in the Beltline before guests arrive this weekend. Approx 900 sq ft. Budget around $150."

That's enough to get accurate quotes. You don't need to be detailed — businesses will ask follow-up questions if they need more info.

**What do you need done?** Post it now: [swingby.ca/app/post](https://swingby.ca/app/post)

Most Calgary jobs on SwingBy get their first quote within 30 minutes.

— The SwingBy team

---

## Email 3 — Day 7 (if no job posted)

**Subject:** What's stopping you?

**Preview text:** We get it. Here are the questions clients ask before posting their first job.

---

Hi {{first_name}},

A few common questions from new SwingBy clients — in case one of these is holding you back:

**"What if I don't know my exact budget?"**
Give a rough range. You can adjust based on quotes. Businesses are used to this.

**"What if my job is complicated?"**
Post it anyway. Experienced businesses will ask the right questions and quote accordingly.

**"What if no one responds?"**
We'll let you know within 24 hours if there aren't enough businesses in your area for your job type. We'll also suggest alternatives.

Still have a question? Reply to this email — we read them all.

Or just try it: [swingby.ca/app/post](https://swingby.ca/app/post)

— The SwingBy team

---

## Email 4 — Day 14 (if first booking made, send on day 14 after booking)

**Subject:** How was your first SwingBy experience?

**Preview text:** Your feedback helps us make the marketplace better.

---

Hi {{first_name}},

You recently completed a booking on SwingBy with {{business_name}}.

If you haven't left a review yet, it takes 30 seconds and it matters a lot — both for the business and for the next client who needs a reliable recommendation.

**Leave your review:** [swingby.ca/app/bookings/{{booking_id}}](https://swingby.ca/app/bookings/{{booking_id}})

Did something go wrong? Reply to this email and we'll sort it out.

— The SwingBy team

---

## Email 5 — Day 30 (retention)

**Subject:** Your next job on SwingBy

**Preview text:** Ready for another? Here are the most popular services this month in Calgary.

---

Hi {{first_name}},

You've been on SwingBy for a month. Here's what's popular in Calgary right now:

- House cleaning (bi-weekly bookings are up this month)
- Lawn care (spring aeration season)
- Dog walking (recurring weekly bookings available)
- Handyman work (pre-summer maintenance rush)

**Post your next job:** [swingby.ca/app/post](https://swingby.ca/app/post)

If you had a great experience with a business from your last booking, you can favourite them and message them directly for future work.

— The SwingBy team

*You're receiving this because you signed up at swingby.ca. [Unsubscribe]({{unsubscribe_url}})*
